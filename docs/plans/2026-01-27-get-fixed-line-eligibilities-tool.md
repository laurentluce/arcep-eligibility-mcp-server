# get_fixed_line_eligibilities Tool Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement the `get_fixed_line_eligibilities` MCP tool that takes a street address and town name, chains 4 ARCEP API calls, and returns fixed-line eligibility data.

**Architecture:** Single file `src/index.ts` contains the MCP server, API client logic, and tool registration. Reads `ARCEP_API_KEY` from environment. Chains: getTown → getAddress → getBuilding → getFixedLineEligibilities. Uses native fetch.

**Tech Stack:** TypeScript, `@modelcontextprotocol/sdk`, Node.js native fetch

---

### Task 1: Implement get_fixed_line_eligibilities tool
...
git commit -m "feat: implement get_fixed_line_eligibilities tool with ARCEP API integration"
```
